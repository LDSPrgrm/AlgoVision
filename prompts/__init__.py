...
def get_prompt_scene_vision_storyboard(
    scene_number: int,
    topic: str,
    description: str,
    scene_outline: str,
...
def get_prompt_scene_technical_implementation(
    scene_number: int,
    topic: str,
    description: str,
    scene_outline: str,
    scene_vision_storyboard: str,
    relevant_plugins: List[str],
...
def get_prompt_scene_animation_narration(
    scene_number: int,
    topic: str,
    description: str,
    scene_outline: str,
    scene_vision_storyboard: str,
    technical_implementation_plan: str,
    relevant_plugins: List[str],
) -> str:
...
def get_prompt_code_generation(
    topic: str,
    description: str,
    scene_outline: str,
    scene_implementation: str,
    proto_tcm: str,  # MODIFIED: Added proto_tcm parameter
    scene_number: int,
    additional_context: Union[str, List[str]] = None,
) -> str:
    """
    ...
    Args:
        ...
        proto_tcm (str): The Proto-TCM JSON string with timing information.
    ...
    """
    # MODIFIED: Added proto_tcm to the format() call
    prompt = _prompt_code_generation.format(
        ...
        proto_tcm=(
            proto_tcm
            if proto_tcm
            else "No timing information provided. Use default animation durations."
        ),
        scene_number=scene_number,
    )
    if additional_context is not None:
...
def get_prompt_fix_error(
    implementation_plan: str,
    manim_code: str,
    error: str,
    proto_tcm: str,
    additional_context: Union[str, List[str]] = None,
) -> str:  # MODIFIED: Added proto_tcm parameter
    """
    Generate a prompt to fix errors in the given manim code.

    Args:
        implementation_plan (str): The implementation plan of the scene.
        manim_code (str): The manim code with errors.
        error (str): The error message encountered.
        proto_tcm (str): The Proto-TCM JSON string with timing information.
        additional_context (Union[str, List[str]], optional): Additional context for the prompt.
    ...
    prompt = _prompt_fix_error.format(
        ...
        proto_tcm=(
            proto_tcm
            if proto_tcm
            else "No timing information provided. Preserve existing run_time values."
        ),
    )
    if additional_context is not None:
        if isinstance(additional_context, str):
...
def get_prompt_visual_fix_error(implementation: str, generated_code: str) -> str:
    prompt = _prompt_visual_fix_error.format(
        implementation=implementation, generated_code=generated_code
    )
    return prompt
...
def get_prompt_rag_query_generation_vision_storyboard(
    scene_plan: str, relevant_plugins: str
) -> str:
    prompt = _prompt_rag_query_generation_vision_storyboard.format(
        scene_plan=scene_plan, relevant_plugins=relevant_plugins
    )
    return prompt


def get_prompt_rag_query_generation_technical(
    storyboard: str, relevant_plugins: str
) -> str:
    ...
    prompt = _prompt_rag_query_generation_technical.format(
        storyboard=storyboard, relevant_plugins=relevant_plugins
    )
    return prompt


def get_prompt_rag_query_generation_narration(
    storyboard: str, relevant_plugins: str
) -> str:
...
def get_prompt_rag_query_generation_code(
    implementation_plan: str, relevant_plugins: str
) -> str:
    ...
    prompt = _prompt_rag_query_generation_code.format(
        implementation_plan=implementation_plan, relevant_plugins=relevant_plugins
    )
    return prompt


def get_prompt_rag_query_generation_fix_error(
    error: str, code: str, relevant_plugins: str
) -> str:
    prompt = _prompt_rag_query_generation_fix_error.format(
        error=error, code=code, relevant_plugins=relevant_plugins
    )
    return prompt


def get_prompt_context_learning_scene_plan(examples: str) -> str:
    prompt = _prompt_context_learning_scene_plan.format(examples=examples)
    return prompt


def get_prompt_context_learning_vision_storyboard(examples: str) -> str:
    prompt = _prompt_context_learning_vision_storyboard.format(examples=examples)
    return prompt


def get_prompt_context_learning_technical_implementation(examples: str) -> str:
    prompt = _prompt_context_learning_technical_implementation.format(examples=examples)
    return prompt


def get_prompt_context_learning_animation_narration(examples: str) -> str:
    prompt = _prompt_context_learning_animation_narration.format(examples=examples)
    return prompt


def get_prompt_context_learning_code(examples: str) -> str:
    prompt = _prompt_context_learning_code.format(examples=examples)
    return prompt


def get_prompt_detect_plugins(
    topic: str, description: str, plugin_descriptions: str
) -> str:
    """
    ...
    prompt = _prompt_detect_plugins.format(
        topic=topic, description=description, plugin_descriptions=plugin_descriptions
    )
    return prompt


def get_prompt_animation(
    topic: str, description: str, additional_context: Union[str, List[str]] = None
) -> str:
    prompt = _prompt_animation_simple.format(topic=topic, description=description)
    if additional_context is not None:
        if isinstance(additional_context, str):
            prompt += f"\nAdditional context: {additional_context}"
        elif isinstance(additional_context, list) and additional_context:
...
def get_prompt_animation_fix_error(
    ...
    prompt = _prompt_animation_fix_error.format(
        text_explanation=text_explanation, manim_code=manim_code, error_message=error
    )
    if additional_context is not None:
        if isinstance(additional_context, str):
            prompt += f"\nAdditional context: {additional_context}"
...
def get_prompt_animation_rag_query_generation(
    topic: str, context: str, relevant_plugins: str
) -> str:
    if context is None:
        context = ""
    prompt = _prompt_animation_rag_query_generation.format(
        topic=topic, context=context, relevant_plugins=relevant_plugins
    )
    return prompt


def get_prompt_animation_rag_query_generation_fix_error(
    text_explanation: str, error: str, code: str
) -> str:
    prompt = _prompt_animation_rag_query_generation_fix_error.format(
        text_explanation=text_explanation, error=error, code=code
    )
    return prompt


def get_prompt_scene_proto_tcm(
    i,
    topic,
    ...
    prompt = _prompt_scene_proto_tcm.format(
        i=i,
        topic=topic,
