...
_code_disable = """manim-code-utils
pip install manim-code-utils"""
...
_code_limit = """Note that the frame width and height are 16.0 and 9.0 respectively. And the center of the frame is (0, 0, 0).
It means to avoid putting any object out of the frame, you should limit the x and y coordinates of the objects.
limit x to be within -8.0 and 8.0 for objects, and limit y to be within -4.5 and 4.5 for objects.
...
**PRESCRIBED TIMELINE (PROTO-TCM):**
**This is the most critical instruction.** You MUST follow this timeline exactly. The `estimatedDuration` from each event in this JSON object is the required `run_time` for the corresponding animation in your code.
{proto_tcm}
...
_prompt_fix_error = """You are an expert Manim Community Edition v0.18.1 developer specializing in debugging and error resolution. Based on the provided implementation plan and Manim code, analyze the error message to provide a comprehensive fix and explanation.
...
- **Pedagogical Narration Script:**
    ...
    - **The script should be written as if delivered by a knowledgeable and engaging lecturer.** It should:
        ...
        - **Narrate what is happening visually while explaining the underlying concepts - never leave silent gaps.**
...
_prompt_scene_proto_tcm = """You are an expert AI assistant that synthesizes video planning documents into a structured, timed event map.

Your task is to create a Proto Temporal Context Map (Proto-TCM) in JSON format based on the provided plans for Scene {i}.
The Proto-TCM must be a JSON list of objects. Each object represents a single, fine-grained pedagogical event with continuous narration.

**CRITICAL: Every TCM entry MUST have narration text. Do NOT create entries for standalone animations without voiceover. The narration should be continuous throughout the scene, with the teacher explaining concepts while animations happen. Break the narration into natural segments that align with visual changes, but ensure there are NO gaps or pauses in the narration flow.**
...
_prompt_visual_fix_error = """You are an expert in Manim Community Edition v0.18.1 animations. Your task is to ensure that the rendered animation frame (image) aligns with the intended teaching content based on the provided implementation plan.
