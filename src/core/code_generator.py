...
import re
import json
from typing import Union, List, Dict, Tuple  # Import Tuple
...
from utils.utils import (
    ...
    _extract_code,
    _prepare_text_image_inputs,
)
from utils.gemini import GeminiWrapper
from prompts import (
...
class CodeGenerator:
    ...
    def __init__(
        ...
        manim_docs_path="rag/manim_docs",
        ...
        self.context_examples = (
            self._load_context_examples() if use_context_learning else None
        )
    ...
    def _load_context_examples(self) -> str:
        ...
        for example_file in glob.glob(
            f"{self.context_learning_path}/**/*.py", recursive=True
        ):
    ...
    def _generate_rag_queries_code(
        ...
        cache_dir = os.path.join(
            self.output_dir,
            re.sub(r"[^a-z0-9_]+", "_", topic.lower()),
        ...
        if relevant_plugins:
            prompt = get_prompt_rag_query_generation_code(
                implementation, ", ".join(relevant_plugins)
            )
        else:
            prompt = get_prompt_rag_query_generation_code(
                implementation, "No plugins are relevant."
            )

        queries = self.scene_model(
    ...
    def _generate_rag_queries_error_fix(
        ...
        prompt = get_prompt_rag_query_generation_fix_error(
            ...
            code=code,
            relevant_plugins=(
                ", ".join(relevant_plugins)
                if relevant_plugins
                else "No plugins are relevant."
            ),
        )
    ...
    def _extract_code_with_retries(
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                print(
                    f"Attempt {attempt + 1}: Failed to extract code pattern. Retrying..."
                )
                # Regenerate response with a more explicit prompt
                response_text = self.scene_model(
                    _prepare_text_inputs(
                        retry_prompt.format(
                            pattern=pattern, response_text=response_text
                        )
                    ),
                    metadata={
        ...
        raise ValueError(
            f"Failed to extract code pattern after {max_retries} attempts. Pattern: {pattern}"
        )
    ...
    def generate_manim_code(
    ...
    ) -> Tuple[str, str]:  # MODIFIED: Corrected return type hint
        ...
        # MODIFIED: Pass the proto_tcm to the prompt generation function
    ...
    def fix_code_errors(
        ...
        """
        # MODIFIED: Pass proto_tcm to the prompt generation function
        ...
        if self.use_rag:
            ...
            # MODIFIED: Pass proto_tcm to the RAG-enabled prompt generation as well
            prompt = get_prompt_fix_error(
                implementation_plan=implementation_plan,
                manim_code=code,
                error=error,
                proto_tcm=proto_tcm,
                additional_context=retrieved_docs,
            )
    ...
    def visual_self_reflection(
        ...
        with open(
            "task_generator/prompts_raw/prompt_visual_self_reflection.txt", "r"
        ) as f:
